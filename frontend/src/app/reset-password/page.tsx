...
export default function ResetPasswordPage() {
  ...
  const verifyToken = async (tokenStr: string) => {
    ...
    try {
      const res = await fetchApi(`/users/verify-reset-token/${tokenStr}`);
      ...
      setIsTokenValid(false);
      setError('Link Sudah Expired , Silahkan Request Lagi !');
    } finally {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl border border-slate-100">
        <div>
          ...
          {isTokenValid && !isValidating && (
            <p className="mt-2 text-center text-sm text-slate-500">
              {searchParams.get('token') ? 'Masukkan password baru Anda' : 'Masukkan token reset dan password baru Anda'}
            </p>
          )}
        </div>
        ...
        {isValidating ? (
          <div className="flex justify-center items-center py-8">
            <p className="text-slate-500">Memverifikasi tautan...</p>
          </div>
        ) : !isTokenValid ? (
          <div className="mt-8 space-y-6">
            <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-4 rounded-lg relative text-center" role="alert">
              <span className="block mb-4">{error}</span>
              <Link href="/forgot-password" className="inline-block px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors">
                Minta Link Baru
              </Link>
            </div>
          </div>
